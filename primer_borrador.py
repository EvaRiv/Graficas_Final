...
vtx=[vtx1,vtx2,vtx3,vtx4,vtx5,vtx6,vtx7,vtx8,vtx9]
for dic in vtx:
    for key in dic:
        arr=dic[key]
        arr[0]=120-arr[0]
...
for dic in vtx:
    for key in dic:
        arr=dic[key]
        arr=[round(x*val,2) for x in arr]
        dic[key]=arr
...
def timer(a):
    global Xi,Yi,Zi,rotXi,rotYi,rotZi,rotLxi,rotLyi,rotLzi,lines,rotCx,rotCz,rad,colorShift,coloraux
    glutPostRedisplay()
    rotCx=math.cos(rad)*17
    rotCz=math.sin(rad)*17
    coloraux=(coloraux+1)%100
    colorShift=round(coloraux/100,1)
    rad+=0.017
    rotYi +=0.5
